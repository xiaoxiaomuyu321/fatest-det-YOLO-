...
class DetectorLoss(nn.Module):
    ...
    def forward(self, preds, targets):
        # 初始化loss值
        cls_loss = torch.tensor([0.0], dtype=torch.float32, device=self.device)
        iou_loss = torch.tensor([0.0], dtype=torch.float32, device=self.device)
        obj_loss = torch.tensor([0.0], dtype=torch.float32, device=self.device)
        # 定义obj和cls的损失函数
        BCEcls = nn.NLLLoss()
        # smmoth L1相比于bce效果最好
